...
(@) Advantages of using purrr functions instead of the apply family in Base R
    - The way arguments are taken in purrr functions is more consistent, e.g. in the purrr functions it seems to be that they take in data first and then the function, but in the base R functions this order varies
...
## Date fields and stuff

### Modifications to the data frames
...
news_date_modifications <- function(news_df) {
  ...
  news_df <- news_df |>
    mutate(pub_diff = lag(publishedAt))
